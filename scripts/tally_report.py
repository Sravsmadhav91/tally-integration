"""
tally_report.py -- a cached SQL reporting layer over the Tally HTTP-XML gateway.

WHY: fetching balances live for a big multi-year book is slow and can hang Tally (see quirks #5/#5a).
Instead this pulls the Voucher Register for a date range ONCE into a local SQLite cache, then runs any
number of reports instantly as SQL. Refreshing a report = re-run for a new --from/--to. Adding a report =
add a SQL string (or drop a .sql file in reports/).

USAGE
  # list built-in reports
  python tally_report.py --list
  # a ready-made report, refreshed for a timeframe (cache auto-built & reused)
  python tally_report.py --company "My Co" --from 20250401 --to 20260331 --report trial-balance
  # a report that needs an argument (e.g. one ledger's statement, with running balance)
  python tally_report.py --company "My Co" --from 20250401 --to 20260331 --report ledger --arg "Bank A/c"
  # ad-hoc SQL over the cached tables (the "query-database" idea)
  python tally_report.py --company "My Co" --from 20250401 --to 20260331 --sql "SELECT vtype,COUNT(*) FROM vouchers GROUP BY vtype"
  # save to CSV / Excel instead of console
  python tally_report.py ... --report stock --out stock.xlsx
  # force a fresh pull from Tally (otherwise a matching cache is reused)
  python tally_report.py ... --report trial-balance --refresh

CACHE TABLES (query these in --sql or your own .sql):
  vouchers(id, date, vtype, remoteid, narration)
  entries(voucher_id, ledger, dr, cr)          -- dr/cr are positive; one row per ledger leg
  inventory(voucher_id, stockitem, in_out, qty, amount)
  masters(name, parent)                        -- ledger -> group (for group-summary)

Deps: requests (+ openpyxl only if you use --out *.xlsx). Company must be loaded; gateway on 9000.
"""
import sys, os, re, sqlite3, csv, argparse, glob

URL = os.environ.get("TALLY_URL", "http://localhost:9000")
CACHE_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), ".cache")
REPORTS_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), "reports")

# ---- built-in reports: name -> SQL. Use :arg for a parameter (pass via --arg). --------------------
BUILTIN = {
...
def _post(xml, timeout):
    import requests
    return requests.post(URL, data=xml.encode("utf-8"),
                         headers={"Content-Type": "text/xml"}, timeout=timeout).text
...
def _masters(company, timeout):
    # light: NAME + PARENT only (no balances) -> never hangs
    xml = ('<ENVELOPE><HEADER><VERSION>1</VERSION><TALLYREQUEST>Export</TALLYREQUEST><TYPE>Collection</TYPE>'
           '<ID>LP</ID></HEADER><BODY><DESC><STATICVARIABLES>'
           '<SVEXPORTFORMAT>$$SysName:XML</SVEXPORTFORMAT>'
           f'<SVCURRENTCOMPANY>{company}</SVCURRENTCOMPANY></STATICVARIABLES>'
           '<TDL><TDLMESSAGE><COLLECTION NAME="LP" ISMODIFY="No"><TYPE>Ledger</TYPE>'
           '<NATIVEMETHOD>PARENT</NATIVEMETHOD></COLLECTION></TDLMESSAGE></TDL></DESC></BODY></ENVELOPE>')
    return _post(xml, timeout)

def _groups(company, timeout):
    # light: group -> parent group (for resolving each ledger's primary P&L group)
    xml = ('<ENVELOPE><HEADER><VERSION>1</VERSION><TALLYREQUEST>Export</TALLYREQUEST><TYPE>Collection</TYPE>'
           '<ID>GP</ID></HEADER><BODY><DESC><STATICVARIABLES>'
           '<SVEXPORTFORMAT>$$SysName:XML</SVEXPORTFORMAT>'
           f'<SVCURRENTCOMPANY>{company}</SVCURRENTCOMPANY></STATICVARIABLES>'
           '<TDL><TDLMESSAGE><COLLECTION NAME="GP" ISMODIFY="No"><TYPE>Group</TYPE>'
           '<NATIVEMETHOD>PARENT</NATIVEMETHOD></COLLECTION></TDLMESSAGE></TDL></DESC></BODY></ENVELOPE>')
    return _post(xml, timeout)
...
def build_cache(company, frm, to, db_path, timeout):
    ...
    for block in re.split(r"(?=<VOUCHER[ >])", vr):
        if not block.startswith("<VOUCHER"):
            continue
        # keep only in-range dates (the VR filter is sometimes ignored — quirks #6)
        ...
        # ledger legs: each leg-block has one LEDGERNAME + ISDEEMEDPOSITIVE + AMOUNT
    ...
    # masters (best-effort; light query)
    try:
        mt = _masters(company, min(timeout, 90))
        ...
        print(f"  (masters not cached: {type(e).__name__}; group-summary may be blank)", file=sys.stderr)
    try:
        gt = _groups(company, min(timeout, 90))
        for m in re.finditer(r'<GROUP NAME="([^"]*)"[^>]*>(.*?)</GROUP>', gt, re.S):
            par = re.search(r"<PARENT[^>]*>(.*?)</PARENT>", m.group(2))
            db.execute("INSERT INTO groups VALUES(?,?)", (_unesc(m.group(1)), _unesc(par.group(1)) if par else ""))
    except Exception as e:
        print(f"  (groups not cached: {type(e).__name__}; pnl may be incomplete)", file=sys.stderr)
    db.commit()
...
def rpt_bank_recon(db, a):
    """Diff a bank ledger's book movement against a statement CSV, by NET DAILY movement (quirks #22)."""
    if not (a.arg and a.statement):
        raise SystemExit('bank-recon needs --arg "<bank ledger>" and --statement <csv>')
    book = {}
...
def rpt_tds_26as(db, a):
    """Book TDS-credit ledgers vs a 26AS CSV (Active only; Inactive/duplicate rows excluded)."""
    if not a.form26as:
...
COMPUTED = {
    ...
    "bank-recon":  (rpt_bank_recon, "diff a bank ledger vs a statement CSV (--arg <ledger> --statement <csv>)"),
    "tds-vs-26as": (rpt_tds_26as,   "book TDS ledgers vs a 26AS CSV (--form26as <csv>)"),
}

def all_reports():
    r = dict(BUILTIN)
    for p in glob.glob(os.path.join(REPORTS_DIR, "*.sql")):
        r[os.path.splitext(os.path.basename(p))[0]] = open(p, encoding="utf-8").read().strip()
    return r

def main():
    ap = argparse.ArgumentParser(description="Cached SQL reporting over the Tally gateway.")
    ...
    ap.add_argument("--list", action="store_true"); ap.add_argument("--timeout", type=int, default=300)
    a = ap.parse_args()
    reports = all_reports()
    if a.list:
        print("SQL reports:")
        for k in sorted(reports):
            print(f"  {k}{'   (needs --arg)' if ':arg' in reports[k] else ''}")
    ...
    if a.sql:
        ...
        if a.report not in reports:
            ap.error(f"unknown report {a.report!r}; use --list")
        sql = reports[a.report]
